...
class ActionsDiscretizer(gym.ActionWrapper):
    ...
    def __init__(self, env):
        ...
        # buttons = ["MODE", "START", "UP", "DOWN", "LEFT", "RIGHT", "A", "B"]
        actions = [['RIGHT', 'B','A'],['RIGHT'],['RIGHT','B'],['A']]#, ['RIGHT', 'C', 'X'],['RIGHT', 'C', ''],['RIGHT','Y'],['RIGHT','A']]
        self._actions = []
...
def make_env(env_idx):
    ...
    env = make_retro(game=dicts[env_idx]['game'], state=dicts[env_idx]['state'])#, bk2dir="./records")#record='/tmp')
...
def _read_mem_range(env, address, length):
        """
